...
To separate \emph{learned sharing} from the network's per-trace inference we trained an
identical architecture with the session-attention block removed, so that each trace is
estimated independently, with the same training data and budget range
(\nSeedsAblation{} seeds). In the primary region the ablated model is worse than the
session-conditioned one at every budget above $\rMinK$k
($\rmseAblFiveK$, $\rmseAblTwentyK$, $\rmseAblEightyK$ and $\rmseAblThreeTwentyK$~nT at
$\rMinK$k, $20$k, $80$k and $320$k, against $\rmseNetSetFiveK$, $\rmseNetSetTwentyK$,
$\rmseNetSetEightyK$ and $\rmseNetSetThreeTwentyK$~nT respectively), and it also loses the
sharp transition at high budget where the conditioned model overtakes every classical
estimator. The session context is therefore doing real work: the network's advantage is not
a generic property of amortized regression but comes from information shared across the
columns of a session.
