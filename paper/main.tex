...
lowest budget, where the statistical error is $59$~nT at best), so the baseline is not
start-limited at the precision that matters here.
...
own reference. The per-trace estimator lies $1.8$--$2.2\times$ above the joint bound at the lowest
budgets and $1.6\times$ at the highest; the pooled estimator sits within $10\%$ of the joint
bound up to $80$k and rises to $2.0\times$ above it at the highest budget, where the small
residual floor of Eq.~\eqref{eq:law} takes over.}
\label{fig:bounds}
\end{figure*}
...
field range) recovers a large part of the difference, but not all of it: freeing $T_2^{*}$
per column in the full-sweep session moves the floor from $\lawFullB$ to $95$~nT and the
crossover from $\lawFullRstar$ to $6.9\times10^{5}$ repetitions, so the remaining spread is
carried by the other shared parameters.
...
inference is ahead of it by $11$--$21\%$ at the four lowest budgets
($\rmseNetSetEightyK$ against $\rmsePoolEightyK$~nT at $80$k is the worst case) and per-trace
fitting is ahead at $\rMaxK$k ($\rmseNetSetSixFortyK$ against $\rmseLmSixFortyK$~nT). What
