...
the four estimators; error bars are percentile bootstrap intervals over the $34$ columns
(for the network, the interquartile spread over training seeds). Dashed and dotted curves
...
gap is the factor $\approx\!1.8$ that pooling is worth. Right: the measured RMSE of each
estimator divided by its own bound. Both estimators sit at their own bound for
$r\le320$k; the pooled estimator deteriorates above that, which is the bias floor of
Eq.~\eqref{eq:law} expressing itself as an apparent inefficiency.}
\label{fig:bounds}
\end{figure*}
...
prescribes without the law being supplied to it; it does not, however, yet match the
classical pooled fitter in the regime where aggressive sharing is optimal, and we report
that shortfall rather than presenting the network as uniformly superior.
