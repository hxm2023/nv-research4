...
$10\%$ of the joint bound at most budgets, with excursions to $1.3\times$ at $20$k and
$160$k, and rises to $2.0\times$ at the highest budget, where the small residual floor of
Eq.~\eqref{eq:law} takes over.}
\label{fig:bounds}
\end{figure*}
...
field range) recovers about $40\%$ of the difference, but not all of it: freeing $T_2^{*}$
...
inference is ahead of it at every budget below $\rMaxK$k, by $3.5$--$21\%$
($\rmseNetSetEightyK$ against $\rmsePoolEightyK$~nT at $80$k is the worst case, and the two
are within $4\%$ at $20$k), and per-trace fitting is ahead at $\rMaxK$k
...
little: full and partial pooling differ by $4.4$~nT of floor (Table~\ref{tab:ladder}), so
sharing the relaxation time there is harmless. In the full sweep, by contrast, freeing the
relaxation time removes $64$~nT of floor (Table~\ref{tab:session} and the text above). Applying
