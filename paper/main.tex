...
$\rMinK$k repetitions against a free-envelope bound of $\crlbFreeFiveK$~nT), yet is a
factor $\ratioLmPoolFiveK$ worse than the pooled estimator whose bound is lower because the
instrument's shared nuisance structure, one relaxation envelope and one phase frame for a
whole field sweep, is estimated once for the whole sweep rather than once per trace. Both estimators obey a two-term law
...
before the measurement. An amortized neural estimator trained once across the whole budget
range, without being told the instrument model or which parameters are shared, stays within
$2$--$21$ percent of the best per-budget classical estimator at every budget --- a
consistency that no single classical estimator achieves --- although it is never itself the
best. The practical rule that follows is not
...
sampling: the sweep is fixed and only the photon budget varies. Companion work on the same
apparatus addresses the complementary question --- which delay times to measure, and what
aliasing that choice costs~\cite{L3companion,L4companion} --- while the present study holds
the sweep fixed and varies only the number of repetitions.
...
$25$ changes the fitted fields by a median of $0.03$~nT and by at most $39$~nT at the
lowest budget --- an order of magnitude below the $\rmseLmFiveK$~nT error obtained
there --- so the baseline is not start-limited at the precision that matters here.
...
bound) between $1.0$ and $1.3$ for $r\le320$k and $0.90$ at $\rMaxK$k. The classical fitter
is therefore essentially not improvable by a better optimiser. The bound is evaluated with
each column's own fitted envelope and the measured noise law; the $10\%$ sub-unity value at
$\rMaxK$k is within the uncertainty of those inputs and is not interpreted as
super-efficiency. Session pooling attains $\rmsePoolFiveK$~nT against a joint bound of
...
a heavy tail there; the pooled estimator remains ahead at $80$k and $160$k
($p=\pJointVsLmEightyK$ and $\pJointVsLmOneSixtyK$). Above that the two become
statistically indistinguishable ($p=\pJointVsLmThreeTwentyK$ at $320$k and
$\pJointVsLmSixFortyK$ at $\rMaxK$k). We therefore present the ladder as a curve described by a two-parameter
...
the five estimators (three least-squares variants, the marginal-posterior estimator, and the
network). Shaded bands are \emph{variability}, not confidence intervals: for the classical
estimators they span a $2000$-resample percentile bootstrap over the $34$ columns, which
quantifies how consistently an ordering holds across field settings within this single
session and not a sampling uncertainty of the method; for the network the band is the
interquartile spread across the five training seeds. Dashed and dotted curves
...
remains ahead of per-trace fitting at every budget up to $320$k repetitions and falls behind
by $23\%$ at $\rMaxK$k ($\rmsePoolSixFortyK$ against $\rmseLmSixFortyK$~nT); the two bias
curves in the lower panel of Fig.~\ref{fig:ladder} stay comparable over the whole ladder.
...
own reference. The per-trace estimator lies $1.6$--$2.2\times$ above the joint bound at every budget
(the ratio has no clear trend with photon budget); the pooled estimator lies within
$10\%$ of the joint bound at four of the eight budgets, with excursions to $1.3\times$ at
$20$k and $160$k, $1.6\times$ at $320$k and $2.0\times$ at the highest budget, where the
small residual floor of Eq.~\eqref{eq:law} takes over.}
\label{fig:bounds}
\end{figure*}
...
($T_2^{*}$, which varies by $\sigma=\envTstarStd\mus$, i.e.\ $5\%$ of its mean, over the
field range) recovers $65\%$ of the difference, but not all of it: freeing $T_2^{*}$
per column in the full-sweep session moves the floor from \lawFullB~nT to
$\lawFullTFreeB$~nT and the crossover from \lawFullRstar{} to
$\lawFullTFreeRstar$ repetitions, so part of the penalty is carried by the other shared
parameters and by the unidentifiable settings themselves.
...
pool will be dragged by the records that carry the least information.

The synthetic control of Fig.~\ref{fig:control} tests the corresponding conclusion with
exact ground truth. It is a \emph{full-sharing} control --- one shared envelope for a session
of randomly drawn field settings, no measurement systematics --- and it reproduces the
full-sharing behaviour rather than the primary-region one: with per-trace
$\ctrlLmFiveK$ against pooled $\ctrlPoolFiveK$~nT at $\rMinK$k and $\ctrlLmSixFortyK$
against $\ctrlPoolSixFortyK$~nT at $\rMaxK$k (medians, as this control reports), its fitted
law has a pooled floor of $\sim\!0.4$~$\mu$T and a crossover near
$3\times10^{4}$ repetitions. Since the synthetic session contains no unidentifiable settings
and no instrument drift, that floor is entirely a property of sharing one envelope across
heterogeneous settings. The primary-region result is therefore a statement about which
settings may enter the pool, and the full-sharing control is the limiting case it avoids.
...
field RMSE against budget for per-trace fitting, full pooling and partial pooling, with
interquartile bands. With no measurement systematics and no unidentifiable settings present,
the pooled estimator still acquires a floor near $0.4$~$\mu$T and is overtaken by per-trace
fitting above $\sim\!3\times10^{4}$ repetitions, which isolates the floor as a property of
sharing one envelope across heterogeneous settings.}
\label{fig:control}
\end{figure}
...
resultant length, the analogue of a correlation coefficient for angles). These scatter
values are the model-mismatch budget that the pooled estimators of
Fig.~\ref{fig:ladder} must absorb.}
\label{fig:env}
\end{figure}
...
(their bounds range from $4.1\times10^{3}$ to $4.9\times10^{4}$~nT, i.e.\ $4$ to $45$ field
steps of $1071.4$~nT) and columns $8$--$40$ as identifiable, so the criterion selects $33$ of
the $\nCols$ settings --- within one column of the $34$ used above. Using the strict
$33$-setting session changes the law modestly --- $A=\lawStrictA$~nT ($+0.7\%$),
$b=\lawStrictB$~nT ($-3.8\%$) and $r^{*}=\lawStrictRstar$ ($+7\%$) against the
$34$-setting values --- so the conclusion does not hinge on the exact cutoff, though the
crossover does move by more than the coefficients because it divides by $b^{2}$.
...
few per cent, and the two least identifiable ones (columns $2$ and $1$, single-trace bounds of $45$ and
$39$ field steps) destroy the joint fit outright, driving $A$ to
...
per-trace and pooled inference, and for the amortized network; the shaded bands show the
per-point overhead $t_{\rm ovh}$ varied over $0.5$--$2$~$\mu$s, which rescales $\etaS$ for
every estimator alike and therefore leaves the ranking unchanged.}
\label{fig:eta}
\end{figure}
...
for per-trace fitting). It is never itself the best estimator at any budget: pooled
inference is ahead of it at every budget below $\rMaxK$k, by $2.2$--$21\%$ across those
seven budgets ($\rmseNetSetEightyK$ against $\rmsePoolEightyK$~nT at $80$k is the worst case;
the two are within $4\%$ at $20$k and within $2.2\%$ at $320$k), and per-trace fitting is
ahead at $\rMaxK$k
($\rmseNetSetSixFortyK$ against $\rmseLmSixFortyK$~nT). What
it does do is stay within $2$--$21\%$ of the best per-budget classical estimator at
\emph{every} budget with a single set of weights, whereas each classical estimator is
itself far off the best somewhere in the ladder --- the pooled fitter by $23\%$ at
$\rMaxK$k and per-trace fitting by $63\%$ at $\rMinK$k. In other words the network has learned to stop sharing once
...
\emph{(ii) The gain has a floor, and where the floor comes from depends on the session.}
Sharing is only as good as the shared model, and our instrument's relaxation envelope does
vary across the field range --- by $5\%$ of its mean in $T_2^{*}$ and by $21\%$ in the
stretch exponent (Fig.~\ref{fig:env}). But that variation is not by itself what costs: in
the identifiability-filtered session, where the floor is only
$b_{\rm pool}=\lawBPool$~nT, releasing $T_2^{*}$ per column changes it by $4.4$~nT, so there
the residual floor is carried by the other shared parameters rather than by the relaxation
time. The envelope inhomogeneity matters through the session instead: pool the full sweep
and the floor rises to $\lawFullB$~nT, of which freeing $T_2^{*}$ recovers $65\%$. Both
terms of Eq.~\eqref{eq:law} are measurable before an experiment --- $A$ from the bound
ratio, $b$ from the envelope scatter and the identifiability of the settings --- so the
crossover is predictable rather than merely empirical, provided the session is chosen
deliberately.
...
into a two-step rule. The prescription is not "pool or do not pool" but \emph{choose the session and the sharing
pattern deliberately}. In the identifiability-filtered session the shared envelope costs
little: full and partial pooling differ by $4.4$~nT of floor (Table~\ref{tab:session}), so
sharing the relaxation time there is harmless. In the full sweep, by contrast, the same
relaxation releases $64$~nT of floor, and it is that session in which sharing the
inhomogeneous parameters is what advances the crossover. Applying
...
fields to within $0.0$~nT at all eight budgets; and the conditioning of the
Jacobian. The raw condition number of $J^{\!\top}J$ is $1.2\times10^{11}$ at every budget,
which is a statement about units --- field parameters in nT against envelope parameters of
order unity --- rather than about the problem: rescaling every column of $J$ to unit norm
gives a dimensionless condition number of $3.99$--$4.00$, identical at all eight budgets.
The fit is therefore well conditioned and contains no flat direction of the kind that
produced the earlier artefact. The pooled solution is
...
$\pm3.2\times10^{3}$~nT window used by the refiners. Synthetic sessions drawn from the same envelope population have a
comparable median initialisation error ($0.9\times10^{3}$~nT over twelve sessions, range
$0.7$--$1.2\times10^{3}$~nT), but a heavier tail: a minority of columns receive an
initialisation that is far worse than the median, and because every estimator in the
comparison shares that initialisation, those columns degrade all of them together. A
synthetic-recovery test through the identical pipeline therefore exercises the
initialisation as much as the estimator, and we report it as such rather than as a
validation of the real-data numbers.
...
\bibitem{L3companion} Companion work on sparse delay-time sampling for the same apparatus
(archived technical report; merged into Ref.~\cite{L4companion}), which establishes the
noise law $\sigma(r)=0.207\sqrt{5000/r}$ used here.
\bibitem{L4companion} Companion work on sampling-design limits --- fold boundaries, the
photon-insensitive failure plateau, and offline design screening --- for the same apparatus
(in preparation).
\end{thebibliography}
